...
//				"Bar.hbm.xml"
	...
	public void hhh18436Test() throws Exception {
		...
		final Level2 child1 = new Level2( root, 1L);
		final Level2 child2 = new Level2( root, 2L);
		final Level2 child3 = new Level2( root, 3L);
		...
		final Level1 loadedWithoutEntityGraph = s.find( Level1.class, 1L);

		long i = 1;
		for ( final Level2 child : loadedWithoutEntityGraph.getChilds()) {
	...
	public void hhh18436TestEntitygraph() throws Exception {
		...
		final Level1 loadedWithEntityGraph = s.find( Level1.class, 1L, hints);

		long i = 1;
		for ( final Level2 child : loadedWithEntityGraph.getChilds()) {
			Assert.assertEquals("Childs not in expected order", Long.valueOf(i), child.getId());
			i++;
		}
