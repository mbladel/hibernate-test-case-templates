...
public class ORMUnitTestCase extends BaseCoreFunctionalTestCase {
	...
	@Test
	public void testConvertedAttributeQLQueryBinding() {
		Session s = openSession();
		Transaction tx = s.beginTransaction();
		...
		TypedQuery<Post> query = s.createQuery(
				"from Post p where concat( ',', p.tags ) like ?1",
				Post.class ).setParameter(
				1,
				"%foo%"
		);
	...
	@Test
	public void testConvertedAttributeQLQueryInline() {
		Session s = openSession();
		Transaction tx = s.beginTransaction();
		...
		TypedQuery<Post> query = s.createQuery(
				"from Post p where concat( ',', p.tags) like '%foo%'",
				Post.class
		);
		List<Post> resultList = query.getResultList();
		Assertions.assertEquals( 1, resultList.size() );
